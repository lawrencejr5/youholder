					...
					<div class="auth-section signin-top d-flex align-items-center">
						<div class="auth-module">
							...
							<form action="" method="post" id="login-form">
								<input type="hidden" id="resend_code" value="<?php echo rand(100000, 999999) ?>" autocomplete="off">
	...
	<script type="text/javascript">
		$(document).ready(() => {
			...
			loginBtn.on('click', () => {
				...
				const resend_code = $('#resend_code').val()
					...
					$.ajax({
						...
						success: (res) => {
							if (res.header == 'error') {
								...
								loginBtn.html('Sending verification code......')
								$.ajax({
									...
									success: (res) => {
										if (res.header == 'resent') {
											setTimeout(() => {
												window.location = `../register/verify.php?em_verify=${email}&fname=${fname}`
											}, 2000)
										}
									}
								})
